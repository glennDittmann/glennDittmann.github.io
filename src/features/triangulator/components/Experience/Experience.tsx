...
import "./styles.css";
import type { Tetrahedron3 } from "../../types/Tetrahedron3";
...
function Vertices({ vertices, color = "#ffffff" }: { vertices: Vertex3[]; color?: string }) {
...
function LiftedVertices() {
  const liftedVertices = useAppSelector((state) => state.liftedVertices.liftedVertices);
...
export default function Experience() {
  ...
  const liftedVertices = useAppSelector((state) => state.liftedVertices.liftedVertices);
  const liftedTriangles = useAppSelector((state) => state.liftedTriangles.liftedTriangles);
  ...
  return (
    ...
    >
      ...
      {vertices.length > 0 && showVertices && <Vertices vertices={vertices} color={vertexColor} />}
      {simplifiedVertices.length > 0 && <Vertices vertices={simplifiedVertices} color="#00ff00" />}
      {liftedVertices.length > 0 && <LiftedVertices />}
      {triangles.length > 0 &&
        triangles.map((triangle: Triangle3) => <Triangle key={triangle.id} triangle={triangle} />)}

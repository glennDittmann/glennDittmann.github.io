...
import initRita, {
  ...
  triangulate3d as ritaTriangulate3D,
} from "@lempf/rita";
import {
 ...
 */
export async function invoke<T>(command: string, args?: { request: unknown }): Promise<T> {
  switch (command) {
    case "triangulate": {
      const request = args?.request as TriangulationRequest;
      try {
        await ensureRita();
        const flat = new Float64Array(request.vertices.flatMap((v) => [v.x, v.z]));
    ...
    case "tetrahedralize": {
      ...
      try {
        await ensureRita();
        const flat = new Float64Array(request.vertices.flatMap((v) => [v.x, v.z, v.y]));
    ...
    case "simplify2d": {
      try {
        if (!currentClusterer2D) {
          throw new Error("simplify2d requires an existing clusterer; run cluster2d first");
        }
